<!-- 
Purpose: Edit Skills Display Form
...
.shwTextCount    { color: #FF0000;  }
	...
	<body onload="document.skillform.skilldetails.focus()">
	<form action="skillssubmit.asp" name="skillform" method=post onsubmit="return validatePage()" >
	<input type=hidden name=redirectflag value="">
	<input type=hidden name=oldcategory value="<%=category%>" >
	<input type=hidden name=oldskillused value=<%=strskillu%> >
   <input type=hidden name=oldskilldetails value="<%=skilldetails%>" >	
	<table border="0" cellspacing="0" width="550">
        ...
        <table width="550" border="0" cellspacing="0" cellpadding="2">
          <tr> 
            <td colspan="2" align="middle" width="567"> 
              <p align="left"> <b> <font size="4" color="#6699CC" face="Arial"> 
                Edit Skills</font> </b> </p>
            </td>
          </tr>
          <tr> 
          ...
          <tr> 
            <td align="right" width="145" class="tabletext">Skill Category</td>
            <td width="405"> 
           
              <input type="text" maxlength=50 name="skillcategory" value="<%=category%>" size="45" >
            </td>
          </tr>
          <tr> 
            <td align="right" width="145" class="tabletext">Skill Details
            <div>(<span id='shwMessage' class="shwTextCount"><%=Len(TheValue("skilldetails"))%></span>/500 
                  chars)</div>
            </td>
            <td width="405"> 
              <textarea align="left" name="skilldetails" onKeyDown="showLength(this.form.skilldetails,'document.all.shwMessage',500)" 
                    onKeyUp="showLength(this.form.skilldetails,'document.all.shwMessage',500)" 
                    onfocus="showLength(this.form.skilldetails,'document.all.shwMessage',500)" maxlength="500" rows="2" cols="38"><%=TheValue("skilldetails")%></textarea>
            </td>
          </tr>
        
          <tr> 
            <td align="right" width="145" class="tabletext"> 
 Last Used<font color="#cc0000">* </font>  </td>
            <td width="405"> 
              <select name="skillused" size=1>
                <OPTION VALUE="<%=TheValue("skillused")%>" 
              selected><%=skillusedval(TheValue("skillused"))%></OPTION>
 ...
 Skill Level<font color="#cc0000">* </font>  </td>
            <td width="405"> 
              <select name="skilllevelid" size=1>
                <OPTION selected VALUE="<%=TheValue("skilllevelid")%>"><%=skilllevelval(TheValue("skilllevelid"))%></OPTION>
 ...
 Experience in Hours<font color="#cc0000">* </font> 
            ...
            <td width="405"> 
              <select name="hoursused" size=1>
                <option selected value="<%=TheValue("hoursused")%>"><%=exphrs(TheValue("hoursused"))%></option>
                ...
                <TR> 
                  <TD colSpan=2 width="550" height="8"> 
                    ...
                    <%
						...
						%> 
                <tr> 
                  <td align=left width="275" height="4" valign="top" nowrap><%=rs2("category")%> 
                  </td>
                  <%skillusedvar=skillusedval(rs2("skillused"))%> 
                  <td align=right valign=top width="275" height="4" nowrap><%=skillusedvar%> 
                  <a href="editskills.asp?Rowid=<%=Server.URLEncode(rs2("RowId"))%>&amp;skilla=Edit">Edit</a> 
                    <A href="deleteskills.asp?Rowid=<%=Server.URLEncode(rs2("RowId"))%>&skilla=Delete"  onclick="return confirm('Are you sure?')">Delete</A>                     
                   
                <tr> 
                  <td align=left valign=bottom colspan="2" width="550"> 
                    <hr noshade size=1>
                  </td>
                </tr>
                <%  
